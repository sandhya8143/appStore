...
class AppStore extends Component {
  state = {searchValue: '', tabItemId: tabsList[0].tabId}
  ...
  onClickTab = tabId => {
    this.setState({tabItemId: tabId})
  }

  onClickTabList = tabItemId => {
    const filterItems = appsList.filter(each => each.category === tabItemId)
    return filterItems
  }

  render() {
    const {tabItemId, searchValue} = this.state
    const filterItems = this.onClickTabList(tabItemId)
    const lastSearch = filterItems.filter(each =>
        ...
        <div className="container">
          <ul className="listItem">
            {lastSearch.map(value => (
              <AppItem appListItem={value} key={value.appId} />
            ))}
          </ul>
        </div>
      </div>
